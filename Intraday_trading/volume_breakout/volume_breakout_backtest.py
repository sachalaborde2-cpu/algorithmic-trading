"""
Backtest d'un Opening Range Breakout (ORB) filtre par le volume, intraday.

Variante de `breakout_opening/orb_backtest.py` : la sortie du range d'ouverture
n'est prise comme signal que si la barre de cassure s'accompagne d'un volume
anormalement eleve (ratio vs volume moyen des `volume_lookback` barres
precedentes >= `volume_ratio_min`). Une cassure a faible volume est ignoree
(on continue a chercher une cassure confirmee plus tard dans la seance), sur
l'hypothese que ca filtre les fausses sorties.

Conventions (identiques a l'ORB de base)
-----------------------------------------
- Barres IB : l'horodatage est le DEBUT de la barre.
- Tout est converti en heure de New York.
- Signal calcule a la CLOTURE d'une barre, execution a l'OUVERTURE de la
  barre suivante. Le ratio de volume ne depend que des barres deja connues
  (jamais de la barre de cassure elle-meme dans son propre denominateur).
- Un seul trade par jour, toujours flat avant la cloture (16:00 NY).

Usage
-----
    python volume_breakout_backtest.py data/MES_5min_all.csv --instrument MES
...
class Instrument:
    tick: float
    point_value: float      # $ par point et par position (1 contrat, ou 100 actions de reference)
    commission_side: float  # $ par position et par cote, tout compris
...
class Params:
    ...
    volume_lookback: int = 12         # barres precedentes utilisees pour le volume moyen
    volume_ratio_min: float | None = None  # None ou <=0 : pas de filtre (= ORB de base)
...
def _volume_ratio(v: list, k: int, lookback: int) -> float:
    """Ratio volume[k] / moyenne des `lookback` barres precedentes (strictement passees)."""
    lo = max(0, k - lookback)
    if lo == k:
        return np.inf
    avg = sum(v[lo:k]) / (k - lo)
...
def simulate_day(day: Day, p: Params, inst: Instrument, bar_minutes: int = 5,
    ...
    Regles :
      ...
      2. On scanne les barres apres le range ; une barre qui CLOTURE hors du range
         n'est retenue comme signal QUE si son ratio de volume (vs les
         `volume_lookback` barres precedentes) >= `volume_ratio_min`. Sinon on
         ignore cette tentative et on continue a chercher une cassure confirmee
         plus tard. Si `volume_ratio_min` est None/<=0, pas de filtre (= ORB de base).
      ...
      5. Stop prioritaire sur l'objectif si les deux sont touches dans la meme barre.
    ...
    need_filter = p.volume_ratio_min is not None and p.volume_ratio_min > 0

    d_sig, k, vol_ratio = 0, None, np.nan
    for k in range(n_or, n - 1):
        if c[k] > or_hi:
            ...
            continue
        ratio = _volume_ratio(v, k, p.volume_lookback)
        if need_filter and ratio < p.volume_ratio_min:
            continue  # cassure non confirmee par le volume, on continue a chercher
        d_sig, vol_ratio = cand, ratio
        break
    if d_sig == 0:
        return None
    ...
    return {
        ...
        "reason": reason, "or_width": or_hi - or_lo, "volume_ratio": vol_ratio,
    }
...
def main() -> None:
    ...
    # 1. Grille de parametres (avec et sans filtre volume, pour comparaison directe)
    grid = [Params(or_minutes=o, stop_mode=s, target_r=t, volume_ratio_min=vr, volume_lookback=12)
            for o, s, t, vr in product((15, 30), ("opposite", "mid"),
                                        (None, 1.5, 2.0), (None, 1.2, 1.5, 2.0))]
    ...
    # 2bis. Comparaison directe : meme OR/stop/target, avec vs sans filtre volume
    baseline = Params(**{**asdict(best), "volume_ratio_min": None})
    tr_base = run_backtest(days, baseline, inst, bm)
    print(f"\n=== 2bis. Meme config sans filtre volume (ORB de base) : {baseline} ===")
    print("IS  :", fmt(metrics(sel(tr_base, is_dates), is_dates)))
    print("OOS :", fmt(metrics(sel(tr_base, oos_dates), oos_dates)))
    ...
    if len(wf_dates):
        ...
        for t1, p in log:
            print(f"  test a partir de {t1} : OR={p.or_minutes}, stop={p.stop_mode}, "
                  f"target={p.target_r}, vol_min={p.volume_ratio_min}")
    else:
    ...
    print(f"  Breakout reel : {actual:.0f}$ | pile ou face : {mu:.0f}$ +/- {sd:.0f}$ | p-value = {pval:.3f}")
